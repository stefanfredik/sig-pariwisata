...
import { Save, UploadCloud, X, LayoutGrid, Info } from 'lucide-vue-next';
...
<template>
    <AdminFormLayout
        ...
        description="Tambahkan fasilitas penunjang untuk meningkatkan kenyamanan objek wisata."
    ...
    >
        <form @submit.prevent="submit" class="space-y-8 pb-12">
            ...
            <Card class="border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl overflow-hidden">
                ...
                <CardContent class="p-6 space-y-6">
                    <div class="space-y-2">
                        ...
                        <Input v-model="form.nama_fasilitas" placeholder="Contoh: Toilet Umum, Parkir Luas" class="rounded-xl border-slate-200 h-11" />
                    ...
                    <div class="space-y-2">
                        ...
                        <Input v-model="form.deskripsi" placeholder="Keterangan tambahan..." class="rounded-xl border-slate-200 h-11" />
            ...
            <Card class="border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl overflow-hidden">
                ...
                <CardContent class="p-6 space-y-6">
                    <div class="space-y-4">
                        <label class="text-xs font-black uppercase tracking-widest text-slate-500 block text-center">Unggah Foto Fasilitas</label>
                        <div class="mt-1 flex justify-center px-6 pt-8 pb-8 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-2xl hover:border-primary transition-all group cursor-pointer bg-slate-50/30">
                            <div class="space-y-2 text-center">
                                <UploadCloud class="mx-auto h-10 w-10 text-slate-300 group-hover:text-primary transition-colors" />
                        ...
                        <!-- Previews -->
                        <div v-if="photoPreviews.length" class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div class="flex items-center gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                <Button type="submit" :disabled="form.processing" class="px-10 font-black uppercase tracking-widest text-xs">
                    <Save class="w-4 h-4 mr-2" />
                    {{ form.processing ? 'Menyimpan...' : 'Simpan Fasilitas' }}
                </Button>
                <Button variant="ghost" class="px-8 font-black uppercase tracking-widest text-xs" as-child>
